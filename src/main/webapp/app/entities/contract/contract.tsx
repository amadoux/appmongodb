  ...
  return (
    <div>
      <h2 id="contract-heading" data-cy="ContractHeading">
        <Translate contentKey="appmongodbApp.contract.home.title">Contracts</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="appmongodbApp.contract.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/contract/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="appmongodbApp.contract.home.createLabel">Create new Contract</Translate>
          </Link>
        </div>
      </h2>
        ...
        {contractList && contractList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="appmongodbApp.contract.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('contractType')}>
                  <Translate contentKey="appmongodbApp.contract.contractType">Contract Type</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('contractType')} />
                </th>
                <th className="hand" onClick={sort('entryDate')}>
                  <Translate contentKey="appmongodbApp.contract.entryDate">Entry Date</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('entryDate')} />
                </th>
                <th className="hand" onClick={sort('releaseDate')}>
                  <Translate contentKey="appmongodbApp.contract.releaseDate">Release Date</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('releaseDate')} />
                </th>
                <th className="hand" onClick={sort('statusContract')}>
                  <Translate contentKey="appmongodbApp.contract.statusContract">Status Contract</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('statusContract')} />
                </th>
                <th className="hand" onClick={sort('uploadContract')}>
                  <Translate contentKey="appmongodbApp.contract.uploadContract">Upload Contract</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('uploadContract')} />
                </th>
                <th>
                  <Translate contentKey="appmongodbApp.contract.employee">Employee</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {contractList.map((contract, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`appmongodbApp.ContractType.${contract.contractType}`} />
                  </td>
                  ...
                  <td>
                    <Translate contentKey={`appmongodbApp.StatusContract.${contract.statusContract}`} />
                  </td>
                  <td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="appmongodbApp.contract.home.notFound">No Contracts found</Translate>
            </div>
          )
        )}
      </div>
