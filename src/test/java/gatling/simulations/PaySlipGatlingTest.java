...
/**
 * Performance test for the PaySlip entity.
 */
public class PaySlipGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all paySlips").get("/api/pay-slips").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new paySlip")
                        .post("/api/pay-slips")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                "\"netSalaryPay\": 0" +
                                ", \"paySlipDate\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"uploadPaySlip\": null" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_paySlip_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created paySlip").get("${new_paySlip_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created paySlip").delete("${new_paySlip_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PaySlip entity").exec(scn);
