...
/**
 * Performance test for the SocialCharges entity.
 */
public class SocialChargesGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all socialCharges").get("/api/social-charges").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new socialCharges")
                        .post("/api/social-charges")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"spentDate\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"spentType\": \"MATERIAL\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_socialCharges_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created socialCharges").get("${new_socialCharges_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created socialCharges").delete("${new_socialCharges_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the SocialCharges entity").exec(scn);
