 ...
 */
public class EnterpriseGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all enterprises").get("/api/enterprises").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new enterprise")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"businessRegisterNumber\": \"SAMPLE_TEXT\"" +
                                ", \"uniqueIdentificationNumber\": \"SAMPLE_TEXT\"" +
                                ", \"businessDomicile\": \"SAMPLE_TEXT\"" +
                                ", \"email\": \"SAMPLE_TEXT\"" +
                                ", \"businessPhone\": \"SAMPLE_TEXT\"" +
                                ", \"country\": \"CAMEROON\"" +
                                ", \"city\": \"SAMPLE_TEXT\"" +
                                ", \"businessLogo\": null" +
                                ", \"mapLocator\": null" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_enterprise_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created enterprise").get("${new_enterprise_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created enterprise").delete("${new_enterprise_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Enterprise entity").exec(scn);
